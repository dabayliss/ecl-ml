<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="Dataset[starts-with(@name,'GOOGLECHART')]" mode="generate_body">
  <div>
  ...
  <xsl:template match="Dataset[starts-with(@name,'GOOGLECHART')]" mode="generate_styles">
    <xsl:for-each select="Row[chartelementtype='STYLES']">
     <xsl:value-of select="s"/>
    </xsl:for-each>
  </xsl:template>

  <xsl:template match="Dataset[starts-with(@name,'GOOGLECHART')]" mode="generate_script">
      <xsl:text>
