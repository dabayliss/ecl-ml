...
*/
  ...
  Split(DATASET(KdNode) nodes, t_level p_level) := FUNCTION
    working_nodes:=nodes(level=p_level);
		...
		splits_mean := PROJECT( sp, TRANSFORM(KdNode,SELF.Id := 0, SELF.level := p_level, SELF.value := (LEFT.maxv+LEFT.minv)/2,SELF := LEFT) );
		...
		s_median := PROJECT( ML.FieldAggregates(into_med).medians, TRANSFORM(KdNode, SELF.Id := 0, SELF.level := p_level, SELF.node_id:=LEFT.number, SELF.value := LEFT.median, SELF.number := 0));
		splits_median := JOIN(s_median,sp,LEFT.node_id=RIGHT.node_id,TRANSFORM(KdNode,SELF.number := RIGHT.number,SELF := LEFT),FEW);
		splits := IF ( p_level < 10, splits_median, splits_mean );
