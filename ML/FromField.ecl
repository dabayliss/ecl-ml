...
EXPORT FromField(dIn,lOut,dOut,dMap=''):=MACRO
  ...
  #ELSE
    #UNIQUENAME(dTmp)
    %dTmp%:=JOIN(dIn,dMap((UNSIGNED)assigned_name>0),LEFT.number=(UNSIGNED)RIGHT.assigned_name,TRANSFORM({UNSIGNED %id%;RECORDOF(dIn) OR RECORDOF(dMap);},SELF.%id%:=LEFT.id;SELF:=LEFT;SELF:=RIGHT;),LOOKUP,LEFT OUTER);
    %dInPrep%:=%dTmp%+PROJECT(DEDUP(dIn,id),TRANSFORM(RECORDOF(%dTmp%),SELF.%id%:=LEFT.id;SELF.orig_name:=dMap(assigned_name='ID')[1].orig_name;SELF.value:=LEFT.id;SELF:=LEFT;SELF:=[]));
  #END
