...
EXPORT Query := MODULE
	...
	*/
	EXPORT GetSimilar(DATASET(Sentilyze.Types.WordType) w) := FUNCTION
		...
		Sentilyze.WordNet.Synset.Layout	GetWordId(Sentilyze.Types.WordType L,Sentilyze.WordNet.Synset.Layout R)	:= TRANSFORM
			SELF.Word	:= L.Word;
			SELF.SynsetId	:= R.SynsetId;
		END;

		WordWithId	:= JOIN(w,Sentilyze.WordNet.Synset.File,LEFT.Word = RIGHT.Word,GetWordId(LEFT,RIGHT));
		...
		SimilarIdLayout	GetSimilarId(Sentilyze.WordNet.Synset.Layout word, Sentilyze.WordNet.Similar.Layout similar) := TRANSFORM
			SELF.Word	:= word.Word;
			SELF.SynsetId1	:= word.SynsetId;
			SELF.SynsetId2	:= similar.SynsetId2;
		END;

		WordWithSimilarId	:= JOIN(WordWithId,Sentilyze.WordNet.Similar.File,LEFT.SynsetId = RIGHT.SynsetId1,GetSimilarId(LEFT,RIGHT));
		...
		WordWithSimilarLayout	GetSimilarWord(SimilarIdLayout L, Sentilyze.WordNet.Synset.Layout R) := TRANSFORM
			SELF.Word	:= L.Word;
			SELF.Similar := R.Word;
		END;

		WordWithSimilar := JOIN(WordWithSimilarId,Sentilyze.WordNet.Synset.File,LEFT.SynsetId2 = RIGHT.SynsetId,GetSimilarWord(LEFT,RIGHT));
